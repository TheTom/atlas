...
impl Qwen3AttentionLayer {
    ...
    pub(super) fn prefill_attention_paged_oproj(
        ...
        } else if self.w4a16_rdna4_k.0 != 0 && self.o_dense_bf16.is_none() {
            // `o_dense_bf16.is_none()` because this rung jumps AHEAD of the
            // BF16-dense arm below, and a model whose o_proj ModelOpt keeps
            // in BF16 (Gemma-4) has no NVFP4 `attn.o_proj` to read. The
            // twin arms it jumps ahead of are NVFP4-only, so nothing else
            // in the ladder changes meaning.
