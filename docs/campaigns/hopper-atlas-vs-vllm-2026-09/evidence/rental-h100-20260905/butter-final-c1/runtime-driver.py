#!/usr/bin/env python3
"""Separate C1-only follow-up; does not bypass the failed C16 admission."""
import argparse
...
def main():
    ...
    ap.add_argument('--work-cap-s',type=int,default=420)
    args=ap.parse_args()
    red=selftest()
    ...
    absolute_end=datetime.datetime(2026,9,6,1,42,0,tzinfo=datetime.timezone.utc)
    seconds_left=(absolute_end-datetime.datetime.now(datetime.timezone.utc)).total_seconds()
    if seconds_left<120:raise SystemExit('C1 absolute release reserve refuses a late start')
    deadline=min(deadline,started+seconds_left)
    out=ROOT/'results'/('butter-block-fp8-lat-c1-final-'+datetime.datetime.now(datetime.timezone.utc).strftime('%Y%m%dT%H%M%SZ'))
    ...
    def run(name,argv,cap,allowed=(0,)):
        ...
        progress_bytes=0
        progress_pending=''
        def relay_ladder_progress():
            nonlocal progress_bytes,progress_pending
            path=out/'ladder.stdout'
            if not path.exists():return
            with path.open() as f:
                f.seek(progress_bytes);chunk=f.read();progress_bytes=f.tell()
            progress_pending+=chunk
            while '\n' in progress_pending:
                line,progress_pending=progress_pending.split('\n',1)
                event={'observed_utc':utc(),'line':line}
                with (out/'ladder-progress.jsonl').open('a') as log:log.write(json.dumps(event)+'\n')
                print('LADDER_PROGRESS',json.dumps(event),flush=True)
        def audit_ladder_budget():
            ...
            has_c1=re.search(r'C=\s*1 SERIES ',log) is not None
            needs_c16=argv[argv.index('--concs')+1]=='1,16'
            if needs_c16 and c16_started is None and has_c1:
            ...
            if has_c1 and ladder_path.exists():
                try:
        ...
        with (out/(name+'.stdout')).open('wb') as stdout,(out/(name+'.stderr')).open('wb') as stderr:
            ...
            try:
                while p.poll() is None:
                    ...
                    if name=='ladder':relay_ladder_progress();audit_ladder_budget()
                ...
                stop_owned(p)
                if name=='ladder':relay_ladder_progress()
                record.update(exit=p.returncode,elapsed_s=time.monotonic()-start,finished_utc=utc());dump(name+'.command.json',record)
    ...
    try:
        ...
        boot_cap=min(300,max(0,deadline-boot))
        while time.monotonic()-boot<boot_cap:
        ...
        else:raise TimeoutError(f'boot cap{boot_cap:.1f}s, bounded by absolute deadline')
        ...
        gate('pre')
        free=free_memory();require_capacity(free,False);dump('c1-memory-admission.json',{'free_mib':free,'required_mib':8*1024,'utc':utc(),'scope':'Single concurrent request only; prior C16 admission timed out and is not bypassed for C16'})
        remaining=deadline-time.monotonic()-100
        if remaining<60:raise TimeoutError('insufficient remaining window for ladder plus post-gate')
        run('ladder',[str(ATLAS/'vllm/bin/python3'),str(harness),'--url','http://127.0.0.1:8890','--model',str(MODEL),'--label','rental.qwen38.butter.native-lat-c1','--out',str(out/'ladder.json'),'--concs','1','--reps','3','--isl','1024','--osl','256','--warmup','1'],remaining)
        ladder=json.loads((out/'ladder.json').read_text());assert [r['concurrency'] for r in ladder['rungs']]==[1]
        for rung in ladder['rungs']:
            ...
            for rep in rung['reps']:
                ...
                assert rep['completion_tokens_per_req']==[256]*rung['concurrency']
