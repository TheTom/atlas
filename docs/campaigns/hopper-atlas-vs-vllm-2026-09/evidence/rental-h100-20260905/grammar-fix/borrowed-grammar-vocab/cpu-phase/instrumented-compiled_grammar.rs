...
impl CompiledGrammar {
    ...
    fn compute_mask(
        ...
        let mask = generator.get_adaptive_token_mask(is_root);
        eprintln!("MASK rule={} state={} key={:?} ms={:.3}", pimpl.grammar.rule(canonical.rule_id).name, canonical.element_id, pimpl.rule_mask_key(&canonical, is_root).map(|(_,k)|k), started.elapsed().as_secs_f64()*1000.0);
        mask
    }
