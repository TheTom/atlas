...
revision=7c786cc50455dee52c11c3bf4097de945fbb8f6a
out=$root/results/diagnostic.bf16-head.$revision
cd "$root/src/atlas"
...
if /usr/bin/time -v -o "$out/build.time" timeout 300 cargo +1.93.1 build --locked --release -p spark-model --features cuda,gpu-examples --example dense_gemv_bf16_batchm_microtest > "$out/build.stdout" 2> "$out/build.stderr"; then rc=0; else rc=$?; fi
printf '%s\n' "$rc" > "$out/build.exit"
test "$rc" = 0
bin=$root/bin/$revision/qwen3.8-27b/examples
mkdir -p "$bin"
cp "$CARGO_TARGET_DIR/release/examples/dense_gemv_bf16_batchm_microtest" "$bin/"
sha256sum "$bin/dense_gemv_bf16_batchm_microtest" > "$out/binary.sha256"
printf '%s\0' "$bin/dense_gemv_bf16_batchm_microtest" > "$out/numerical.argv"
if /usr/bin/time -v -o "$out/numerical.time" timeout 120 "$bin/dense_gemv_bf16_batchm_microtest" > "$out/numerical.stdout" 2> "$out/numerical.stderr"; then rc=0; else rc=$?; fi
