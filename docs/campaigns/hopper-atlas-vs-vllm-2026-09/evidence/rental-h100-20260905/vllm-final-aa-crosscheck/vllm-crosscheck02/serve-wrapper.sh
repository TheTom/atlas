...
if [ "$engine" = atlas ]; then
  test -n "$atlas_binary"
  test -x "$atlas_binary"
  test "$(basename "$atlas_binary")" = spark
  python3 - "$out/serve.argv" "$atlas_binary" "$timing" <<'PYARGV'
import sys
from pathlib import Path
p=Path(sys.argv[1]); argv=p.read_bytes().split(b'\0'); argv[0]=sys.argv[2].encode()
if sys.argv[3]=='native_fp8':
    if b'--lm-head-dtype' in argv:
        argv[argv.index(b'--lm-head-dtype')+1]=b'bf16'
    else:
        if argv[-1]==b'': argv.pop()
        argv.extend([b'--lm-head-dtype',b'bf16',b''])
p.write_bytes(b'\0'.join(argv))
PYARGV
  sha256sum "$atlas_binary" > "$out/executed-spark.sha256"
fi
cp "$0" "$out/serve-wrapper.sh"
