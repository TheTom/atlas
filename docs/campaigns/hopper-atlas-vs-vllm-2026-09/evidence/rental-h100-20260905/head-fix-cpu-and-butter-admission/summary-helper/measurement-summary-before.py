...
def main():
    ...
    for directory in sorted(args.raw.glob("benchmark.qwen38*")):
        if directory.is_dir() and not (directory / "ladder.json").exists():
            pending.append({"directory": directory.name, "status": "no_ladder_export_yet",
                            "launch": read(directory / "launch-ready.json") or read(directory / "launch.json"),
                            "boot": read(directory / "boot.json"),
                            "concurrent_quality": read(directory / "concurrent-quality/summary.json"),
                            "certification_claimed": False})
    ...
    if matching_pairs:
        ...
        for a, v, kind in matching_pairs:
            ...
            comparisons.append({"name": name, "kind": kind, "native_profile_parity": parity, "atlas_input": a["directory"],
    ...
    for item in pending:
        ...
        lines += [f"- {item['directory']}: no ladder JSON exported. Concurrent-quality control={control.get('passed')}, C16={concurrent.get('passed')}, observed client HTTP overlap={concurrent.get('max_overlapping_client_http_attempts')}. This is a quality diagnostic, not a throughput rung or proof of simultaneous GPU rows."]
    for c in cells:
