...
timing=${3:-off}
case "$timing" in off|decode) ;; *) exit 2 ;; esac
if [ "$timing" = decode ] && [ "$engine" != atlas ]; then exit 2; fi
...
if [ "$timing" = decode ]; then
  python3 - "$out/process-env.json" <<'PYENV'
import json,sys
from pathlib import Path
p=Path(sys.argv[1]); d=json.loads(p.read_text()); d['ATLAS_DECODE_TIMING']='1'; p.write_text(json.dumps(d,indent=2)+'\n')
PYENV
fi
cd "$root/src/atlas"
git rev-parse HEAD > "$out/harness.sha"
sha256sum "$root/bin/spark" > "$out/available-spark.sha256"
printf '%s\n' 'Diagnostic protocol session after both scored coherency gates failed; no certified performance result. Exact prior recipe argv retained; environment recorded, optional decode timing changes logging only.' > "$out/RESTRICTION.txt"
