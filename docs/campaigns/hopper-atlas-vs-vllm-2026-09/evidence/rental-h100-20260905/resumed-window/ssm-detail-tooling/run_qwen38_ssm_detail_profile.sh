...
out=$root/results/diagnostic.qwen38.ssmdetail.$label
test ! -e "$out"
mkdir "$out"
cd "$source"
...
test "$(git rev-parse 'HEAD^{tree}')" = b20f14a38ba36a1ed87a12d14fd3a7d9fa4325cf
...
detail_rows=[]
for line in (out/'serve.log').read_text().splitlines():
    a=re.search(r'ATLAS_SSM_MS_PROFILE n=(\d+): mixer=(\d+)us moe_residual=(\d+)us',line)
    if a:
        phase_rows.append({'n':int(a[1]),'mixer_us':int(a[2]),'ffn_residual_us':int(a[3]),'source_line':line})
    b=re.search(r'ATLAS_SSM_DETAIL n=(\d+): (.*)',line)
    if b:
        detail_rows.append({'n':int(b[1]),'parts_us':{k:int(v) for k,v in re.findall(r'(\w+)=(\d+)us',b[2])},'source_line':line})
phase_groups=[]
detail_groups=[]
for n in sorted({r['n'] for r in phase_rows}):
    selected=[r for r in phase_rows if r['n']==n]
    phase_groups.append({'n':n,'layer_invocations':len(selected),'median_us':{k:statistics.median(r[k] for r in selected) for k in ('mixer_us','ffn_residual_us')}})
for n in sorted({r['n'] for r in detail_rows}):
    selected=[r for r in detail_rows if r['n']==n]
    keys=sorted({k for r in selected for k in r['parts_us']})
    detail_groups.append({'n':n,'layer_invocations':len(selected),'parts':{k:{'samples':sum(k in r['parts_us'] for r in selected),'median_us':statistics.median(r['parts_us'][k] for r in selected if k in r['parts_us'])} for k in keys}})
subphases={'scope':'Nested synchronized wall attribution; inspect timer scope before summing detail labels; no pure-GPU or frozen-throughput claim','phase_rows':phase_rows,'detail_rows':detail_rows,'phase_groups':phase_groups,'detail_groups':detail_groups,'actual_c4_subphases':any(r['n']==4 for r in phase_rows) and any(r['n']==4 for r in detail_rows)}
(out/'ssm-subphase-summary.json').write_text(json.dumps(subphases,indent=2)+'\n')
print(json.dumps({'phase_groups':phase_groups,'detail_groups':detail_groups}))
if not result['actual_c4_observed'] or not subphases['actual_c4_subphases']: raise SystemExit(2)
PY
exit "$quality_rc"
