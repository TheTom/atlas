...
out=$root/results/diagnostic.native-fp8-oproj.$revision
cd "$root/src/atlas"
...
if /usr/bin/time -v -o "$out/build.time" timeout 300 cargo +1.93.1 build --locked --release -p spark-model --features cuda,gpu-examples --example native_fp8_oproj_batch_microtest > "$out/build.stdout" 2> "$out/build.stderr"; then rc=0; else rc=$?; fi
printf '%s\n' "$rc" > "$out/build.exit"
test "$rc" = 0
